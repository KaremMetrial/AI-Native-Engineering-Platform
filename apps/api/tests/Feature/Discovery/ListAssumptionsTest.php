...
class ListAssumptionsTest extends TestCase
{
    use DatabaseTransactions;
    use RegistersViaHttp;

    public function test_lists_assumptions_for_a_session(): void
    {
        ...
        $this->withHeader('Authorization', "Bearer {$token}")
            ->postJson("/api/discovery-sessions/{$sessionId}/assumptions", ['statement' => 'Users have a stored payment method.'])
            ->assertStatus(201);

        $response = $this->withHeader('Authorization', "Bearer {$token}")
            ->getJson("/api/discovery-sessions/{$sessionId}/assumptions");

        $response->assertStatus(200)->assertJsonCount(1, 'assumptions');
        $this->assertSame('Users have a stored payment method.', $response->json('assumptions.0.statement'));
    }
    ...
    public function test_returns_404_for_an_unknown_session(): void
    {
        ...
        $response = $this->withHeader('Authorization', "Bearer {$registration['token']}")
            ->getJson('/api/discovery-sessions/'.((string) Str::uuid()).'/assumptions');
    ...
    {
        $this->getJson('/api/discovery-sessions/'.((string) Str::uuid()).'/assumptions')->assertStatus(401);
    }
